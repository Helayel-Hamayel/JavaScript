...
function platformQuality(posts) {
  const isPlatformQualityGood = posts.every(
...
function filterCatagory(posts, catagory = "gaming") {
  ...
  const filteredCatagory = posts.filter(
    (item) => item.category.toLowerCase() === catagory.toLowerCase(),
  );
...
function findUserByUsername(users, usernameToFind) {
  ...
  const foundUser = users.find(
    (user) => user.username.toLowerCase() === usernameToFind.toLowerCase(),
  );
...
function platformStatstics() {
  ...
  const [highestCat, highestCount] = Object.entries(catItemCount).sort(
    (a, b) => b[1] - a[1],
  )[0];
